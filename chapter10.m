...
trialId = randsample(EEG.trials, 1);

% getting 50 poitns around the middle of the trial
indices = [round((length(EEG.times)/2))-24:1:round((length(EEG.times)/2))+25];

% selecting channel for plotting
channelLabel = "FCz";
channelIndex = strcmpi(channelLabel,{EEG.chanlocs.labels});
